...
\newcommand{\FullSWOFnDUI}{\emph{FullSWOF\_1D/2D/UI}}
...
% Headers and footers
\lhead{Releasing \FullSWOFnDUI{} or \SWASHES{}}
...
\item In the file ``make\_config'', change the first lines into
\begin{verbatim}
CPP=i686-w64-mingw32-g++
CPPFLAGS= -I$(INC_DIRS) -Wall -pedantic -O3 -static
\end{verbatim}
...
\end{verbatim}
\item Test the \_win.zip file under win7.64 and win11 by
...
It was tested under win 7 64 bits and win 11.
...
tested under win 7 64 bits and win 11.
\end{verbatim}
\item Click on ``Submit''.
\end{enumerate}
...
system. The package contains an executable tested under win 7 64 bits and win 11.
...
\item In the file ``Makefile'', change the line
\begin{verbatim}
TARGET_EXEC ?= FullSWOF_1D
\end{verbatim}
into
\begin{verbatim}
TARGET_EXEC ?= FullSWOF_1D_win.exe
\end{verbatim}
\item Start cygwin and go to the main directory.
\item Launch ``make''.\\
...
It was tested on win 7 64 bits and win 11.
For details see file README_win.txt in the _win.zip file.
\end{verbatim}
\item Click on ``Submit/Refresh''.
\item In the section ``Add Files To This Release'', select the \_win.zip file,
...
under win 7 64 bits and win 11.
It is distributed for EVALUATION PURPOSE ONLY.
...
into
\begin{verbatim}
TARGET_EXEC ?= FullSWOF_2D_win.exe
\end{verbatim}
\item Start cygwin and go to the main directory.
\item Launch ``make''.\\
...
exe files tested under win 7 64 bits and win 11.
It is distributed for EVALUATION PURPOSE ONLY.
\end{verbatim}
\item Click on ``Submit''.
\end{enumerate}
...
and FullSWOF_2D exe files tested under win 7 64 bits and win 11.
It is distributed for EVALUATION PURPOSE ONLY.
